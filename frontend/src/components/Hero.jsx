...
const Hero = ({ movie, onPrev, onNext, onEdit, onDelete }) => {
  ...
  return (
    <section className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl mb-8 mt-6 border border-gray-200 dark:border-gray-700">
      <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-8 p-8">
        <img 
          src={movie.movie_photo_path ? `http://localhost${movie.movie_photo_path}` : '/images/default-movie.png'} 
          alt={movie.movie_title} 
          className="rounded-2xl shadow-lg w-56 h-80 object-cover border-4 border-white/40" 
        />
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex gap-3 justify-center md:justify-start mt-4">
            <button onClick={onEdit} className="bg-yellow-400 text-white font-bold py-2 px-6 rounded-full shadow hover:bg-yellow-500 transition">Editar</button>
            <button onClick={onDelete} className="bg-red-600 text-white font-bold py-2 px-6 rounded-full shadow hover:bg-red-700 transition">Eliminar</button>
          </div>
        </div>
      </div>
    </section>
